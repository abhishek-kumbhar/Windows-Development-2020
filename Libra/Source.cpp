...
void DrawDots(HDC hdc) {
	...
	if (flag) {
		for (int i = 0; i < 5000; i++) {
			SetPixel(hdc, rand() % (gRect.right / 2), rand() % (gRect.bottom), RGB(rand() % 255, rand() % 255, rand() % 255));
		}
		flag = 0;
	}
	else {
		for (int i = 0; i < 3000; i++) {
			SetPixel(hdc, rand() % (gRect.right / 2), rand() % (gRect.bottom), RGB(255, 255, 255));
		}
		flag = 1;
	}
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SIZE :
		...
		rs1.left =  gRect.left;
		rs1.top = gRect.top;
		rs1.right = gRect.right / 2;
		rs1.bottom = gRect.bottom;
	
		rs.left = gRect.left + 100;
		rs.top = gRect.top + 100;
		rs.right = gRect.right / 2;
	...
	case WM_PAINT :
		...
		StretchBlt(hdc, gRect.right / 2, 0, gRect.right - 900, gRect.bottom, hdcmem, 0, 0, bitmap.bmWidth, bitmap.bmHeight, SRCCOPY);
