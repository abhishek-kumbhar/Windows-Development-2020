...
void CALLBACK MyTimerProc(HWND, UINT, UINT_PTR, DWORD);	// steps


void CALLBACK MyTimerProc2(HWND, UINT, UINT_PTR, DWORD);	//pole
void CALLBACK MyTimerProc3(HWND, UINT, UINT_PTR, DWORD); //flag
void CALLBACK MyTimerProc4(HWND, UINT, UINT_PTR, DWORD); // ashokachakra
void CALLBACK MyTimerProc5(HWND, UINT, UINT_PTR, DWORD); // flag waving
void CALLBACK MyTimerProc6(HWND, UINT, UINT_PTR, DWORD); 
void CALLBACK MyTimerProc7(HWND, UINT, UINT_PTR, DWORD);
...
TCHAR* str[] = {
	...
	TEXT("भारत भाग्य विधाता!"),
	TEXT("पंजाब-सिंध-गुजरात-मराठा,"),
	TEXT("द्राविड़-उत्कल-बङ्ग"),
	TEXT("विंध्य हिमाचल यमुना गंगा, उच्छल जलधि तरंग"),
	TEXT("तव शुभ नामे जागे,"),
	TEXT("तव शुभ आशिष मांगे"),
	TEXT("गाहे तब जय गाथा।"),
	TEXT("जन-गण-मंगलदायक जय हे,"),
	TEXT("भारत भाग्य विधाता!"),
	TEXT("जय हे! जय हे! जय हे!"),
	TEXT("जय जय जय जय हे!"),
	TEXT("HAPPY "),
	TEXT("INDEPENDENCE"),
	TEXT("DAY"),
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	hwnd = CreateWindow(szAppName,
		TEXT("India Flag 15 August Demo - Abhishek Kumbhar-Omkar Ajagunde"),
		WS_OVERLAPPEDWINDOW,
		200,
		100,
		1400,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE : 
		...
		hFont = CreateFont(60, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_RASTER_PRECIS,
			CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, VARIABLE_PITCH, TEXT("Impact"));

		hFont2 = CreateFont(80, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_RASTER_PRECIS,
			CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, VARIABLE_PITCH, TEXT("Doppio One"));
...
void CALLBACK MyTimerProc(HWND hwnd, UINT iMsg, UINT_PTR timerEvent, DWORD ms) {
	...
	if (sOneStepComplete && sTwoStepComplete && sThreeStepComplete) {
		KillTimer(hwnd, MYTIMER);			// finally kill
		ReleaseDC(hwnd, hdc);
...
void DrawAnthem(HWND hwnd) {
	hdc = GetDC(hwnd);
	static int xStart = gRect.left;
	static int yStart = gRect.top;
	static int yHapppy;
	static int x = 1;
	if (x) {
		x = 0;
		refY = gRect.right / 4;
		yStart = (gRect.bottom - 720) / 2;
		yHapppy = gRect.bottom / 2;
	}
	...
	if (cnt < 12) {
		GetTextExtentPoint32(hdc, str[cnt], wcslen(str[cnt]), &size);
		xStart = refY - (size.cx / 2);
		TextOut(hdc, xStart, yStart, str[cnt], wcslen(str[cnt]));
		yStart += 60;
	}
	else {
		...
		refY = gRect.right / 2 + gRect.right / 4;

		if (cnt == 12) {
			SetTextColor(hdc, RGB(250, 161, 5));	//255, 153, 51
		}
		else if (cnt == 13) {
			SetTextColor(hdc, RGB(255, 255, 255));
		}
		else if (cnt == 14){
			SetTextColor(hdc, RGB(5, 250, 18));	 	//19, 136, 8		
		}
		else if (cnt == 15) {
			yHapppy += 20;
			SelectObject(hdc, hFont2);
			SetTextColor(hdc, RGB(95, 255, 215));
		}

		GetTextExtentPoint32(hdc, str[cnt], wcslen(str[cnt]), &size);
		xStart = refY - (size.cx / 2);
		TextOut(hdc, xStart, yHapppy, str[cnt], wcslen(str[cnt]));
		yHapppy += 80;
	}
	cnt++;
...
void CALLBACK MyTimerProc4(HWND hwnd, UINT iMsg, UINT_PTR timerEvent, DWORD ms) {
	...
	if (isDrawn) {
		...
		SetTimer(hwnd, MYTIMER7, TIME7, MyTimerProc7);	
	}
}



void CALLBACK MyTimerProc7(HWND hwnd, UINT iMsg, UINT_PTR timerEvent, DWORD ms) {
	KillTimer(hwnd, MYTIMER7);
	
	DrawAnthem(hwnd);

	if (cnt >= 12) {
		SetTimer(hwnd, MYTIMER7, TIME8, MyTimerProc7);
	}
	else if (cnt < 12){
		SetTimer(hwnd, MYTIMER7, TIME7, MyTimerProc7);
	}
